...
const images = props.item.getItemImages()
...
<template>
    ...
    <div class="p-4 flex flex-col flex-1">
      ...
      <div class="mt-auto flex items-center justify-between">
        <span class="text-xl font-black text-[#4A2E23] dark:text-[#E5B326]">
          {{ formatPrice(item.getEstimatedValue()) }}
        </span>
  ...
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="showModal" class="fixed inset-0 z-[200] flex items-center justify-center p-4">
        ...
        <div @click="closeModal" class="absolute inset-0 bg-[#4A2E23]/70 backdrop-blur-sm">
        </div>

        <div
          class="relative bg-white dark:bg-[#26211E] border-4 border-[#4A2E23] dark:border-[#E5B326] rounded-[32px] shadow-[12px_12px_0px_#4A2E23] dark:shadow-[12px_12px_0px_#E5B326] w-full max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div class="relative aspect-[4/3] bg-[#FBF5E9] dark:bg-[#1A1614] rounded-t-[28px] overflow-hidden">
            <template v-if="images.length > 0">
              <img :src="images[currentImageIndex].url" :alt="item.getName()"
                class="w-full h-full object-cover transition-opacity duration-300" />

              <button @click="openZoom(images[currentImageIndex].url)"
              ...
              <template v-if="images.length > 1">
                ...
                <div class="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
                  <button v-for="(img, idx) in images" :key="idx" @click="currentImageIndex = idx"
              ...
              <span v-if="images.length > 1"
                class="absolute top-3 left-3 bg-black/40 text-white text-xs font-bold px-2.5 py-1 rounded-full">
          ...
          <div v-if="images.length > 1"
            class="flex flex-wrap gap-2 p-3 bg-[#FBF5E9] dark:bg-[#1A1614] border-t-2 border-[#4A2E23]/10 dark:border-[#E5B326]/10">
            <button v-for="(img, idx) in images" :key="idx" @click="currentImageIndex = idx"
              class="flex-shrink-0 w-16 h-16 rounded-xl overflow-hidden border-2 transition-all duration-200" :class="idx === currentImageIndex
          ...
          <div class="p-6 space-y-4">
            ...
            <div
              class="flex items-center gap-2 p-4 bg-[#FBF5E9] dark:bg-[#1A1614] rounded-2xl border-2 border-[#4A2E23]/10 dark:border-[#E5B326]/20">
            ...
            <div v-if="item.getDescription()">
              <p class="text-xs font-black text-[#4A2E23]/50 dark:text-[#D4C7B0]/50 uppercase tracking-widest mb-2">
                Leírás</p>
  ...
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="showZoom" class="fixed inset-0 z-[300] bg-black flex items-center justify-center" @click="closeZoom">
        ...
        <div class="relative hidden md:block overflow-hidden cursor-crosshair"
          style="max-width: 90vw; max-height: 90vh;" @click.stop @mousemove="onZoomMove" @mouseleave="onZoomLeave"
          ref="zoomContainer">
          <img :src="zoomUrl" :alt="item.getName()"
            class="block object-contain transition-transform duration-100 select-none"
            style="max-width: 90vw; max-height: 90vh;" :style="zoomStyle" draggable="false" />
        </div>

        <div class="md:hidden w-full h-full flex items-center justify-center" style="touch-action: pinch-zoom;"
          @click.stop>
          <img :src="zoomUrl" :alt="item.getName()" class="w-full h-full object-contain select-none"
            draggable="false" />
        </div>
      </div>
    </Transition>
